...
uint8_t oldRecPins;
uint8_t tempRecPins;
uint8_t oldTempRecPins;
uint8_t playPins;
uint8_t switchPosition;
uint8_t seqNum = 3;              // sets the default sequence number, change this ONLY when...
uint16_t seqLength;
uint16_t seqStep;
uint16_t oldStep;
uint8_t triggered;
//uint8_t mute = B11111111;
uint8_t extTap = 1;              // = 0 means external clock, = 1 means internal clock (10 or 11 means event timer mode)
uint8_t rollOneshot = 1;         // = 0 means kepresses roll, = 1 means kepresses are one-shot
uint8_t FSmode = 0;              // selects the default freestyle mode
const uint8_t triggerLength = 8;  // sets how long triggers go HIGH. 10 is probably the longest you want to do, 2 might be too short.
uint8_t resetted;
uint16_t USreset;
uint8_t tapped;
int tapNum = -9;
...
int main() {
    ...
    pinDetect pbTap(p5);         //PIND  B0000000x (breaks when Serial is used)
    //pinDetect(1, OUTPUT);               //PORTD B000000x0 (breaks when Serial is used)

    pinDetect pb1(p6);        //PIND B00000x00   button 1
